...
public:
	...
	typedef std::vector<std::vector<eCell > > tMazeRows;
	 ...
	 */
	tDimension getDimensions() { return m_dim; };
...
private:
	...
	struct tCfgLoc {
		...
		tCfgLoc(int r=0, tCoord c = tCoord()): row(r), coord(c){};
	};
	 ...
	 */
	std::vector<eCell> parseRow(std::string line, int rowIdx);
	 ...
	 */
	void updateCfgLocYZ(tCfgLoc &loc, tDimension dim);
};
